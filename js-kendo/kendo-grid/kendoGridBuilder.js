...
var kendoGridBuilder = (function() {
  ...
  var initGrid = function(gridId, columns, setting) {
    logger.debug("<GRID:initGrid>", gridId, setting, columns);
    var grid = {
  ...
  var generateGrid = function(gridId, response, setting) {
    ...
    logger.debug("<GRID:generateGrid>", gridId, setting, columns);
    var grid = {
      columns: columns
    };
    for (var key in setting_grid) {
  ...
  var generateGridStatic = function(gridId, header, setting) {
    ...
    logger.debug("<GRID:generateGridStatic>", gridId, setting, columns);
    var grid = {
      columns: columns
    };
    for (var key in setting_grid) {
  ...
  return {
    ...
    getHeader: function(gridId, filename, setting, fn_succ) {
      var url =
        "/static/grid_header/" + locale.locale() + "/" + filename + ".json";
      $.getJSON(url)
        ...
        .fail(function(jqxhr, textStatus, error) {
          responseError.cb_req_err_all(jqxhr, gridId, url);
        });
    },
    ...
    reqHeader: function(gridId, url, params, setting, fn_succ) {
      $.ajax({
        ...
        error: function(response) {
          responseError.cb_req_err_all(response, gridId, url);
        }
      });
    },
    ...
    reqColumns: function(gridIds, url, params, fn_succ) {
      ...
      $.ajax({
        ...
        error: function(response) {
          responseError.cb_req_err_all(response, gridIds, url);
        }
      });
    },
    ...
    getColumns: function(gridIds, filename, fn_succ) {
      ...
      $.getJSON(url)
        ...
        .fail(function(jqxhr, textStatus, error) {
          responseError.cb_req_err_all(jqxhr, gridIds, url);
        });
    },
    ...
    reqModel: function(gridId, modelUrl, params, setting, fn_succ, bindObjs) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          overlayUtil.end(gridId);
          bindUtil.on(bindObjs);
        },
        error: function(response) {
          logger.debug_ajax_res_fail(gridId + " Model Error ", response);
          overlayUtil.end(gridId);
          responseError.cb_req_err_all(response, gridId, modelUrl);
          bindUtil.on(bindObjs);
        }
      });
    }
