...
var kendoGridCtrl = (function() {
	...
	var selectRow = function(grid, index) {
		...
		if (typeof index === "number") {
			...
			try {
				...
				logger.warn("FAIL selectRow : fail to select grid row");
				return false;
			}
		}
		if (typeof grid == "string") $("#" + grid).data("kendoGrid").select("tr:eq(" + nIndex + ")");
	...
	var selectFirstMatch = function(grid, k, v) {
		...
		if (!(grid.select())) {
			logger.warn("FAIL selectFirstMatch : fail to select grid row");
		} else {
	...
	var selectedRowsIndexs = function(gridId) {
		var currentSelection = $("#" + gridId).data("kendoGrid").select();
		var selectedRows = [];
	...
	var kendoGridToCSV = function(grid) {
		...
		for (var i = 0; i < grid.columns.length; i++) {
			var title = grid.columns[i].title, field = grid.columns[i].field;
